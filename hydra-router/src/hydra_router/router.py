...
Enhanced ZeroMQ router for distributed message routing between clients and servers.
Provides centralized message routing with comprehensive validation, client management,
and error handling.
"""
...
from .router_constants import RouterConstants, RouterLabels
from .validation import MessageValidator, validate_message
...
class MessageRouter:
    ...
    async def _route_client_message(
        ...
        if not servers:
            ...
            error_msg = {RouterConstants.ERROR: "No server connected"}
...
class HydraRouter:
    """Enhanced ZeroMQ router for distributed message routing."""
    ...
    def __init__(
        ...
        """
        Initialize the Hydra Router with enhanced capabilities.
        ...
        try:
            ...
            self.logger.info(f"Hydra Router started on {router_service}")
...
def main():
    """Main entry point for the Hydra Router."""
    parser = argparse.ArgumentParser(
        description="Hydra Router - Routes messages between clients and servers",
...
Examples:
  ...
  ai-hydra-router --address 192.168.1.100 --port 5556
    ...
    print(f"🚀 Starting Hydra Router on {args.address}:{args.port}")
