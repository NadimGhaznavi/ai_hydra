...
from ai_snake_lab.utils.MQClient import MQClient
...
ALL_SIMS = ["left", "right", "straight"]


class SimMgr:
    """Manage multiple concurrent simulations. Publish one"""
    ...
    def __init__(self, router_addr=None):
        ...
        # Manages sending/receiving messages and the connection to the SimRouter
        self.mq_client = MQClient(router=router_addr, loglevel=loglevel)

        # Three simulation servers:
        self.sims = {"left": {}, "right": {}, "straight": {}, "prod": {}}
        ...
        for sim in self.sims.keys():
            self.sims[sim]["game_board"] = ServerGameBoard(DSim.BOARD_SIZE)
            self.sims[sim]["snake_game"] = SnakeGame(
                game_board=self.sims[sim]["game_board"]
            )
            self.sims[sim]["agent"] = AIAgent(db_mgr=self.db_mgr, seed=seed)
            self.sims[sim]["move_delay"] = 0
            self.sims[sim]["highscore"] = 0
            self.sims[sim]["full_throttle"] = False
            self.sims[sim]["config"] = {}
            self.sims[sim]["highscore"] = 0
            self.reset_config(sim)
    ...
    async def handle_requests(self):
        ...
        while True:
            try:
                frames = await self.mq_client.recv_multipart()
            ...
            if elem == DMQ.STATUS:
                ...
                if data == 0:
                    if not self.sims["prod"]["full_throttle"]:
                        self.log.info("Enabling full throttle mode")
                    self.sims["prod"]["full_throttle"] = True
                else:
                    if self.sims["prod"]["full_throttle"]:
                        self.log.info("Disabling full throttle mode")
                    self.sims["prod"]["full_throttle"] = False

            elif elem == DMQ.DYNAMIC_TRAINING:
                for sim in self.sims.keys():
                    self.sims[sim]["agent"].dynamic_training(enable_flag=data)
                    self.sims[sim]["config"][DMQ.DYNAMIC_TRAINING] = True
                    await self.send_ack()

            elif elem == DMQ.EPSILON_DECAY:
                for sim in self.sims.keys():
                    self.sims[sim]["agent"].explore.epsilon_decay(epsilon_decay=data)
                    self.sims[sim]["config"][DMQ.EPSILON_DECAY] = True
                    await self.send_ack()

            elif elem == DMQ.EPSILON_INITIAL:
                for sim in self.sims.keys():
                    self.sims[sim]["agent"].explore.initial_epsilon(
                        initial_epsilon=data
                    )
                    self.sims[sim]["config"][DMQ.EPSILON_INITIAL] = True
                    await self.send_ack()

            elif elem == DMQ.EPSILON_MIN:
                for sim in self.sims.keys():
                    self.sims[sim]["agent"].explore.epsilon_min(epsilon_min=data)
                    self.sims[sim]["config"][DMQ.EPSILON_MIN] = True
                    await self.send_ack()

            elif elem == DMQ.EXPLORE_TYPE:
                for sim in self.sims.keys():
                    self.sims[sim]["agent"].set_explore(explore_type=data)
                    self.sims[sim]["config"][DMQ.EXPLORE_TYPE] = True
                    await self.send_ack()
                ...
                for sim in self.sims.keys():
                    self.sims[sim]["agent"].trainer.set_learning_rate(
                        learning_rate=data
                    )
                    self.sims[sim]["config"][DMQ.LEARNING_RATE] = True
                await self.send_ack()

            elif elem == DMQ.MODEL_TYPE:
                for sim in self.sims.keys():
                    self.sims[sim]["agent"].model_type(model_type=data)
                    self.sims[sim]["config"][DMQ.MODEL_TYPE] = True
                await self.send_ack()

            elif elem == DMQ.MEM_TYPE:
                for sim in self.sims.keys():
                    self.sims[sim]["agent"].memory.mem_type(mem_type=data)
                    self.sims[sim]["config"][DMQ.MEM_TYPE] = True
                await self.send_ack()
                ...
                self.sims["prod"]["config"][DMQ.MOVE_DELAY] = True
                self.sims["prod"]["move_delay"] = float(data)
                ...
                if data == DMQ.START:
                    ...
                    # Reset the game
                    for sim in self.sims.keys():
                        self.sims[sim]["snake_game"].reset()
                        self.sims[sim]["epoch"] = 0
                        model = self.sims[sim]["agent"].model()
                        model.reset_parameters()
                        self.sims[sim]["highscore"] = 0
                        self.reset_config(sim=sim)
    ...
    async def send_ack(self):
        ...
        await self.mq_client.send(msg)

    async def send_error(self, msg):
        await self.mq_client.send(mq_srv_msg(DMQ.ERROR, msg))
    ...
    async def run_simulation(self):
        ...
        game_board = self.sims["prod"]["game_board"]
        agent = self.sims["prod"]["agent"]
        snake_game = self.sims["prod"]["snake_game"]
        ...
        await self.mq_client.send(mq_srv_msg(DMQ.FOOD, mq_loc_data[DMQ.FOOD]))
        ...
        try:
            ...
            while not self.stop_event.is_set():
                ...
                good_move = None
                for sim in self.sims.keys():
                    old_state = self.sims[sim]["game_board"].get_state()
                    move = self.sims[sim]["agent"].get_move(old_state, score)
                    reward, game_over, score = self.sims[sim]["snake_game"].play_step(
                        move
                    )

                    if game_over:
                        self.loser_moves.append()

                # Initialize to all moves
                good_moves = ["left", "right", "straight"]

                # Remove loser moves from the good moves_list
                for sim in self.loser_moves:
                    print(f"Move {sim} lost the game, removing it")
                    good_moves.remove(sim)

                old_state = game_board.get_state()

                if len(good_moves) == 0:
                    # There are no good moves, execute a bad move...
                ...
                if not game_over:
                    ...
                    if not self.full_throttle:
                        ...
                        await self.mq_client.send(
                            mq_srv_msg(DMQ.FOOD, mq_loc_data[DMQ.FOOD])
                        )

                        # Send the score
                        await self.mq_client.send(mq_srv_msg(DMQ.SCORE, str(score)))
                ...
                # Game is over
                    ...
                    if not self.full_throttle:
                        ...
                        elapsed_secs = (datetime.now() - start_time).total_seconds()
                        runtime_str = minutes_to_uptime(elapsed_secs)
                        await self.mq_client.send(mq_srv_msg(DMQ.RUNTIME, runtime_str))

                        # Current epoch
                        await self.mq_client.send(mq_srv_msg(DMQ.EPOCH, str(epoch)))
                        ...
                        # Move delay
                        await self.mq_client.send(
                            mq_srv_msg(DMQ.MOVE_DELAY, self.move_delay)
                        )
                        ...
                        if self.agent.dynamic_training():
                            ...
                            loops = 1
                        await self.mq_client.send(mq_srv_msg(DMQ.TRAINING_LOOPS, loops))
