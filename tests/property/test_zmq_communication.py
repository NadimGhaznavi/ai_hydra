...
from ai_hydra.zmq_protocol import (
    ...
    GameStateData, PerformanceMetrics
)
...
class TestZMQProtocolProperties:
    ...
    @given(
        ...
        request_id=st.text(min_size=1, max_size=100, alphabet=st.characters(whitelist_categories=('Lu', 'Ll', 'Nd', 'Pc', 'Pd'))),
    ...
    def test_zmq_message_protocol_integrity_property(self, client_id, request_id, 
        ...
        **Validates: Requirements 13.1, 13.2**
        ...
        for msg_type in message_types:
            ...
            assert isinstance(json_str, str), f"Serialization should produce string for {msg_type}"
            ...
            assert deserialized_message.client_id == original_message.client_id, \
                f"Client ID should be preserved for {msg_type}"
            assert deserialized_message.request_id == original_message.request_id, \
                f"Request ID should be preserved for {msg_type}"
            
            # Property 3: All data fields and types should be preserved
            self._verify_data_integrity(original_message.data, deserialized_message.data, msg_type)
            ...
            if msg_type == MessageType.START_SIMULATION:
                assert is_valid, f"Deserialized START_SIMULATION message should be valid: {error}"
    ...
    def test_game_state_serialization_integrity_property(self, snake_head_x, snake_head_y,
                                                        snake_length, food_x, food_y,
                                                        score, moves_count, is_game_over):
        """
        ...
        **Validates: Requirements 13.1, 13.2**
        ...
        assert game_state_data["snake_head"] == [snake_head_x, snake_head_y], \
            "Snake head position should be preserved"
        assert len(game_state_data["snake_body"]) == snake_length, \
            f"Snake body length should be preserved: expected {snake_length}, got {len(game_state_data['snake_body'])}"
        assert game_state_data["food_position"] == [food_x, food_y], \
            "Food position should be preserved"
        assert game_state_data["score"] == score, \
            "Score should be preserved"
        assert game_state_data["moves_count"] == moves_count, \
            "Moves count should be preserved"
        assert game_state_data["is_game_over"] == is_game_over, \
            "Game over status should be preserved"
        
        # Verify performance metrics integrity
        perf_data = deserialized_message.data["performance"]
        assert abs(perf_data["decisions_per_second"] - 2.5) < 0.001, \
            "Performance metrics should be preserved with precision"
        assert abs(perf_data["memory_usage_mb"] - 150.0) < 0.001, \
            "Memory usage should be preserved with precision"
    
    @given(
        error_code=st.integers(min_value=1000, max_value=9999),
        error_message=st.text(min_size=1, max_size=200),
        timestamp=st.floats(min_value=1000000000, max_value=2000000000),  # Valid Unix timestamps
        client_id=st.text(min_size=1, max_size=50, alphabet=st.characters(whitelist_categories=('Lu', 'Ll', 'Nd')))
    )
    @settings(max_examples=10, deadline=2000)
    def test_error_message_serialization_property(self, error_code, error_message, timestamp, client_id):
        """
        ...
        **Validates: Requirements 13.1, 13.2**
        ...
        assert error_data_received["error_code"] == error_code, \
            "Error code should be preserved"
        assert error_data_received["error_message"] == error_message, \
            "Error message should be preserved"
        assert abs(error_data_received["timestamp"] - timestamp) < 0.001, \
            "Timestamp should be preserved with precision"
        assert error_data_received["server_info"]["version"] == "1.0.0", \
            "Server info should be preserved"
        assert abs(error_data_received["server_info"]["uptime_seconds"] - 3600.5) < 0.001, \
            "Server uptime should be preserved with precision"
    
    def _verify_data_integrity(self, original_data: dict, deserialized_data: dict, msg_type: str) -> None:
        """Verify that all data fields and types are preserved after serialization."""
        assert type(original_data) == type(deserialized_data), \
            f"Data type should be preserved for {msg_type}"
        
        if isinstance(original_data, dict):
            assert set(original_data.keys()) == set(deserialized_data.keys()), \
                f"Dictionary keys should be preserved for {msg_type}"
            
            for key in original_data.keys():
                self._verify_data_integrity(original_data[key], deserialized_data[key], f"{msg_type}.{key}")
        
        elif isinstance(original_data, list):
            assert len(original_data) == len(deserialized_data), \
                f"List length should be preserved for {msg_type}"
            
            for i, (orig_item, deser_item) in enumerate(zip(original_data, deserialized_data)):
                self._verify_data_integrity(orig_item, deser_item, f"{msg_type}[{i}]")
        
        elif isinstance(original_data, float):
            assert abs(original_data - deserialized_data) < 0.001, \
...
class TestZMQServer:
    ...
    def test_performance_metrics_update(self):
        ...
        assert abs(server.performance_metrics.decisions_per_second - 2.0) < 0.1  # ~10 decisions / 5 seconds
